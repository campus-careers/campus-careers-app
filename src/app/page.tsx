        ...
        <Col xs={10} md={6}>
          <div className="d-grid gap-3">
            <Link href="/company" passHref legacyBehavior>
              <Button variant="success" size="lg" className="w-100">🌟 Explore Company Profiles</Button>
            </Link>
            <Link href="/filter" passHref legacyBehavior>
              <Button variant="primary" size="lg" className="w-100">🔎 Browse By Skill/Location</Button>
            </Link>
            <Link href="/setup" passHref legacyBehavior>
              <Button variant="warning" size="lg" className="w-100 text-white">🛠️ Start Your Profile</Button>
            </Link>
          </div>
        </Col>
      </Row>
        ...
        <Col xs={10} md={8}>
          ...
          <Row xs={1} md={3} className="g-4">
            {[
              ...
              { name: 'Hawaiian Airlines IT Division', description: 'Flying high with cutting-edge IT solutions.' },
              { name: 'Inovatech', description: 'Building the next generation of tech leaders.' },
            ].map((company) => (
              <Col key={company.name}>
                <Card className="h-100">
                  <Card.Body>
                    <Card.Title>{company.name}</Card.Title>
                    <Card.Text className="text-muted">{company.description}</Card.Text>
                  </Card.Body>
                </Card>
              </Col>
            ))}
